...
void *IN_thread(void *param){
	...
	do {
		// returning first empty item in buffer
		...
		while (index > -1){
			...
			// critical section to read from file and store to buffer
...
// worker thread - processes the items in buffer and prepares them for consume
void *worker_thread(void *param){
	...
	do {
		...
		if (index > -1){
			...
			// write prepared item back to buffer, indicate it's ready to be outputted
			result[index].data = curr;
