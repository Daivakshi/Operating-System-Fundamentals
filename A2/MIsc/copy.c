...
void *OUT_thread(void * param){
	...
	while (1){
		...
		if (ftell(file_out) < 0) {
            pthread_mutex_unlock(&mutex);
    		fprintf(stderr, "OUT: Cannot seek output file\n");
			exit(1);
		}
		if ((fwrite(&curr, sizeof(curr), 1, file_out)) < 1) {
            pthread_mutex_unlock(&mutex);
        ...
        //write operation details to log file
        pthread_mutex_lock(&mutexLOG);
        fprintf(file_log,"write_byte CT%d O%d B%d I-1", threadID, (unsigned int)offset, (unsigned int)curr);
        pthread_mutex_unlock(&mutexLOG);
...
int main(int argc, char *argv[]){
	...
	// threads
	pthread_t INthreads[nIN]; //= (pthread_t*)malloc(sizeof(pthread_t)*nIN);
	pthread_t OUTthreads[nOUT]; //= (pthread_t*)malloc(sizeof(pthread_t)*nOUT);
	pthread_attr_t attr;
    ...
    /*for (j = 0; j < nOUT; j++){
        if (pthread_join(OUTthreads[j], NULL) != 0) {
            perror("Failed to join OUT thread");
        }
	}*/
