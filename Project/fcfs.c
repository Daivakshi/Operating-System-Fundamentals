...
// The process array containing the four homogenous CPU's
...
void runningToWaiting(void){
 ...
 //Updates waiting processes, ready processes and running processes (CPUs)
...
//print all the resulting values
...
int main(){
	int i;
	int status = 0;     //to track status of processes when read
	float ut, wt, tat;  //utilisation time, waiting time and turnaround time respectively
	int lastPID;

	// loop clears CPU'S and initializes queues
	for (i = 0; i < NUMBER_OF_PROCESSORS; i++){
		CPUS[i] = NULL;
	}
